...
export class GameComponent implements OnInit {
    ...
    onSelect(n: string): void {
        if (this.isPlayerOne) {
            ...
            this.counter++;
            this.setGuessService(Number(n), "test", this.counter-1);
            this.rateControl.reset();
    ...
    setGuessService(guessNumber: number, distanceRange: string, counter: number): void{
        this.guessService.setGuessNumber(guessNumber, counter);
        this.guessService.setDistanceRange(distanceRange, counter);
    }
}
